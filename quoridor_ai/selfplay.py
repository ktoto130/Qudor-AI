...
def batched_selfplay(net,device,games=64,max_plies=220,batch_cap=256):
 ...
 while not all(done):
  ...
  for off in range(0,len(ids),batch_cap):
   ...
   x=torch.from_numpy(encode_batch(ss)).to(device,non_blocking=True,memory_format=torch.channels_last)
   ...
   for j,i in enumerate(chunk):
    a=acts[j];v=logits[j,a].float().cpu().numpy();temp=1 if states[i].ply<20 else .25;v=v/max(temp,.05);v-=v.max();p=np.exp(v);p/=p.sum();choice=int(np.random.choice(a,p=p));pi=np.zeros(ACTION_SIZE,np.float16);pi[a]=p.astype(np.float16);tr[i].append((encode_batch([states[i]])[0].astype(np.float16),pi,states[i].player));states[i]=apply_unchecked(states[i],choice)
